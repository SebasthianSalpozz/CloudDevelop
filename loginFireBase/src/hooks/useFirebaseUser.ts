...
export const useFirebaseUser = () => {
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
            setUser(currentUser);
            if (currentUser) {
                // Verificamos si el documento del usuario ya existe para no sobrescribirlo
                ...
                if (!userDoc.exists()) {
                    ...
                    try {
                        await setDoc(userDocRef, {
                            ...
                            following: [], // Inicializamos 'following' como un array vacío
                            createdAt: serverTimestamp(),
                        });
                        console.log('Documento de usuario inicializado:', currentUser.uid);
                    } catch (error) {
